...
public class MiniGptWss {
    ...
    public void onMessage(String messages, @PathParam("token") String token, @PathParam("model") String model) {
        try {
            final GptDto gptDto = JSONObject.parseObject(messages, GptDto.class);
            // 微信文字识别能力 防止用户发送色情 政治信息
            weChatUtils.filterText(gptDto.getPrompt(), UserUtils.getOpenIdByToken(token));
            //校验用户次数
            ...
            final GptModel gptModel = new GptModel().setMessages(chatUtils.conversionStructure(gptDto));
