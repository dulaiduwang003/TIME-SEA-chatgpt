 ...
 */
@Slf4j
@ServerEndpoint("/gpt-web/preview/api/{token}/{model}")
@SuppressWarnings("all")
@Service
public class PreviewGptWss {
    /**
     * {
     ...
     *             "type": "image_url",
     *             "image_url": {
     *               "url": "https://hd2-test-ai.oss-cn-shanghai.aliyuncs.com/painting/2d68997e-08a1-439f-9085-ed8d14b77097.jpg",
     *               "detail": "low"
     *             }
     *           }
     *         ]
    ...
    public void onMessage(final String message, final @PathParam("token") String token, final @PathParam("model") String model) {
        try {
            final GptPreviewDto gptPreviewDto = JSONObject.parseObject(message, GptPreviewDto.class);
            ...
            final String s = chatUtils.previewDrawingCueWord(gptPreviewDto.getMessages());

            if (s == null) {
                boolean isPreviewModel = AiTypeConstant.PREVIEW.equals(model);

                final Long frequency = controlCommon.getControl().getEnableGptPlus() ? (isPreviewModel ? chatGptStructure.getGptPlusFrequency() : chatGptStructure.getGptFrequency()) : chatGptStructure.getGptFrequency();

                chatUtils.deplete(frequency, userId);
                //与GPT建立通信
                gptService.concatenationGpt(chatUtils.conversionStructure(gptPreviewDto), isPreviewModel, chatGptStructure)
                        ...
                        .subscribe(data -> {
                            if (JSON.isValid(data)) {
                                ...
                                if (choices != null && !choices.isEmpty()) {
                                    JSONObject delta = choices.getJSONObject(0).getJSONObject("message");
