<template>
  <div class="body" ref="scrollRef">
    <div v-if="!conversationList.length" class="explain">
      ...
      <div class="consume">
        ...
        <div class="consumeText">绘图每次提问消耗30个SUPER币</div>
      </div>
    ...
    <div class="footer">
      <div class="footer-bar">
        ...
        <div
            class="clear3"
            v-show="store.getters.userinfo && !aiLoading"
            @click="drawingDialogueDisplay = true"
        >
          <div style="padding-top: 4px">
            <el-icon size="13px" style="padding-right: 3px">
              <ChatDotRound/>
        ...
        <InputFormField
            ...
            :drawingShowOptions="showDrawing"
            :aiLoading="aiLoading"
            :inputText="input"
      ...
      <div class="cache-content">
        <div class="cache-scrollbar">
          <el-scrollbar height="250px">
            <div
                class="cache-padding"
                v-for="(item, index) in drawingDialogueCache.array"
                :key="index"
            >
              <div class="cache-flex-space-between">
                ...
                <div class="cache-selected">
                  <img
                      :src="
                      drawingDialogueCache.index === index
  ...
  <el-dialog
      v-model="drawingDialogueDisplay"
      title="选择分辨率"
      width="430px"
      center
      style="background-color: var(--bgColor1)"
  >
    ...
    <div class="cache-text">当前分辨率: {{ selectedResolution || '1024x1024' }}</div>
    <div class="cache-flex-center cache-padding-top"></div>
    <div class="file-center">
      <div class="flex">
        <el-button @click="setResolution('1024x1024')" color="#626aef" :dark="isDark" plain>1024x1024</el-button>
        <el-button @click="setResolution('1792x1024')" color="#626aef" :dark="isDark" plain>1792x1024</el-button>
        <el-button @click="setResolution('1024x1792')" color="#626aef" :dark="isDark" plain>1024x1792</el-button>
      </div>
    </div>
...
export default {
  ...
  methods: {conversionTime,
    setResolution(resolution) {
      ...
      this.drawingDialogueDisplay = false; // 关闭对话框
    },
  },
  components: {
    StarFilled,
  ...
  data() {
    return {
      ...
      showDrawing: true, // 或根据逻辑设置为 false
      selectedResolution: '', // 存储用户选择的分辨率
    };
  },
  ...
  setup() {
    ...
    let model = ref("DRAWING");
    const imageUrl = ref("");
    let dialogueDisplay = ref(false);
    let drawingDialogueDisplay = ref(true);
    let selectedResolution  = ref("1024x1024");
    const drawingDialogueCache = ref({});
    ...
    onMounted(() => {
      ...
      //获取对话缓存数据
      let item = localStorage.getItem("drawingDialogueCache");
      if (store.getters.userinfo) {
        if (!store.getters.userinfo) return (loginVisible.value = true);
        if (item) {
          drawingDialogueCache.value = JSON.parse(item);
          let value = drawingDialogueCache.value;
          conversationList.value = value.array[value.index].context;
          // TODO 滚动到底部
          scrollToTheBottom();
        } else {
          drawingDialogueCache.value = {
            index: 0,
            array: [
              {
                title: "新对话",
          ...
          localStorage.setItem(
              "drawingDialogueCache",
              JSON.stringify(drawingDialogueCache.value)
          );
        }
      }
    });
    ...
    function switchChat(index) {
      drawingDialogueCache.value.index = index;
      conversationList.value = drawingDialogueCache.value.array[index].context;
      localStorage.setItem(
          "drawingDialogueCache",
          JSON.stringify(drawingDialogueCache.value)
      );
    ...
    function clearDialogue(index) {
      if (index !== drawingDialogueCache.value.index) {
        let i = parseInt(drawingDialogueCache.value.index);
        if (index < i) {
          drawingDialogueCache.value.index = i - 1;
        }
        drawingDialogueCache.value.array.splice(index, 1);
      }
      localStorage.setItem(
          "drawingDialogueCache",
          JSON.stringify(drawingDialogueCache.value)
    ...
    function writeDialogue() {
      ...
      let value = drawingDialogueCache.value;
      drawingDialogueCache.value.array[value.index].time = Date.now();
      if (item.length > 0) {
        drawingDialogueCache.value.array[value.index].title = item[
        item.length - 1
            ].user
            .trim()
            .slice(0, 25);
      }
      drawingDialogueCache.value.array[value.index].context = item;
      localStorage.setItem(
          "drawingDialogueCache",
          JSON.stringify(drawingDialogueCache.value)
      );
    ...
    function createdNewChat() {
      drawingDialogueCache.value.array.unshift({
        title: "新对话",
        time: Date.now(),
        context: [],
      });
      drawingDialogueCache.value.index = 0;
    ...
    async function onSubmit() {
      ...
      conversationList.value.push({
        user: "/image"+selectedResolution.value+content,
      });
