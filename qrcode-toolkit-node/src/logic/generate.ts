...
export async function generateQRCode(
  ...
  url: string,
  state: QRCodeGeneratorState = defaultGeneratorState(url)
) {
  if (!outCanvas) return;
  ...
  function getInfo(x: number, y: number): PixelInfo {
    ...
    if (x >= 0 && x < 7 && y >= 0 && y < 7) {
      ...
      let dx = x;
      let dy = y;
      while (qr.isSubMarker(dx, dy)) dx -= 1;
      dx += 1;
      while (qr.isSubMarker(dx, dy)) dy -= 1;
      dy += 1;
      ...
      if (isDark) {
        ...
        if (_pixelStyle === "rounded") {
          ...
          if (bottom != null || left != null)
            corner(
              1,
              bottom && left && bottomLeft && !isBorder ? darkColor : lightColor
            );
  ...
  /* if (state.effect === "crystalize") {
    ...
    const newData = effects.crystalize(
      data,
      state.effectCrystalizeRadius,
      state.seed
    );
  ...
  async function applyBackground() {
    ctx.restore();
    const clone = createCanvas(500, 500);
    clone.width = width;
    clone.height = height;
    clone
      .getContext("2d")!
      .putImageData(ctx.getImageData(0, 0, width, height), 0, 0);
      ...
      if (state.backgroundImage.startsWith("#")) {
        ...
        if (imgRatio < canvasRatio)
          ctx.drawImage(
            ...
            img,
            0,
            (outCanvas.height - outCanvas.width / imgRatio) / 2,
            outCanvas.width,
            outCanvas.width / imgRatio
          );
        else
          ctx.drawImage(
            // @ts-expect-error
            img,
            (outCanvas.width - outCanvas.height * imgRatio) / 2,
            0,
            outCanvas.height * imgRatio,
            outCanvas.height
          );
      }
    }
